    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <input
          ...
          className="w-64 border border-hairline-inv bg-charcoal px-3 py-2 text-sm text-pearl outline-none focus:border-gold-soft"
        />
      </div>
      <p className="mt-2 text-[0.7rem] uppercase tracking-[0.14em] text-stone-light">
        {shown.length} of {all.length} image{all.length === 1 ? '' : 's'}
      </p>
      {msg && <p className="mt-3 text-sm text-[#e6b0b0]">{msg}</p>}

      {all.length === 0 ? (
        <p className="mt-10 max-w-[52ch] text-stone-light">
          No images yet. Upload photographs from a piece&rsquo;s editor and they appear here, reusable across the site.
        </p>
      ) : (
        <ul className="mt-6 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          {shown.map((i) => (
            <li key={i.id} className="border border-hairline-inv bg-charcoal/30">
              ...
              <div className="p-3">
                <p className="truncate text-xs text-pearl" title={filename(i.r2_key)}>{filename(i.r2_key)}</p>
                <p className="mt-1 truncate text-[0.68rem] text-stone-light" title={i.alt || 'No alt text'}>{i.alt || '— no alt —'}</p>
                <div className="mt-2 flex items-center justify-between gap-2 font-[family-name:var(--font-mono)] text-[0.58rem] uppercase tracking-[0.12em]">
                  <span className={i.inUse ? 'text-gold-soft' : 'text-stone-light'}>{i.usage}</span>
                ...
                <button
                  ...
                  className="mt-3 w-full border border-hairline-inv py-1.5 font-[family-name:var(--font-mono)] text-[0.58rem] uppercase tracking-[0.16em] text-[#c98b8b] transition-colors hover:border-[#7a2e2e] disabled:cursor-not-allowed disabled:text-stone disabled:opacity-50"
                >
