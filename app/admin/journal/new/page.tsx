    ...
    <AdminChrome name={session.user.name} csrf={session.csrf}>
      ...
      <form method="POST" action="/admin/api/journal" className="mt-6 max-w-md">
        ...
        <label className="block font-[family-name:var(--font-mono)] text-[0.62rem] uppercase tracking-[0.2em] text-stone">
          Title
          <input name="title" required autoFocus className="mt-2 block w-full border border-hairline bg-white px-3 py-2 text-charcoal outline-none focus:border-gold" />
        </label>
        {error && <p className="mt-3 text-sm text-[#a23a3a]">Enter a title to create the post.</p>}
        <button type="submit" className="mt-6 border border-gold px-4 py-2 font-[family-name:var(--font-mono)] text-[0.62rem] uppercase tracking-[0.2em] text-gold transition-colors hover:bg-gold hover:text-obsidian">
          Create &amp; edit
        </button>
      </form>
    </AdminChrome>
